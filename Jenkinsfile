pipeline{ 
    ...
    stages{
        ...
        stage('run backend server') {
             steps {
                script{ 
                    sh ' nohup python rest_app.py & ${env.CREDS_USR} ${env.CREDS_PSW} '
                }
             }
        }
        stage('run fronted server') {
            steps {
                script {
                    sh ' nohup python web_app.py & ${env.CREDS_USR} ${env.CREDS_PSW}'
                }
            }
        }
        ...
        stage('combined testing') {
            ...
            steps {
                ...
                script {
                    sh 'python combined_testing.py ${env.CREDS_USR} ${env.CREDS_PSW}'
                }
            }
        }    
